...
from ..chat_bots.court_brief_bot import CortBriefBot
from ..models import Profile, Chat, Query, ReplyType
...
class CourtBriefConsumer(WebsocketConsumer):
    def connect(self):
    ...
    def receive(self, text_data):
        try:
            user_profile = Profile.objects.get(user__username=self.scope["user"])
            print("here in receiver start 1", user_profile)
            ...
            if payload["type"] == settings.WEBSOCKET_EVENT_TYPES.QUERY_AND_REPLY.value:
                ...
                for element in history:
                    if element["type"] == "query":
                        ...
                        # I included generated arguments in history so that when system is generating arguments , 
                        # it is aware of cases selected and arguments that were generated from selected cases
                        # but , I am commenting it , because client mentioned that system is not referrering to selected past cases, 
                        # I will fetch selected past cases on run-time during arguments generation , and then append them in prompt so llm is completely aware of case values and argument that was generated
                ...
                conversational_handler = CortBriefBot(_ChatInstance=self)
